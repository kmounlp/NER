"""fasttext Embeddings + chars bi-LSTM + bi-LSTM + CRF"""
...
DATADIR = '../../data/example/NER_ETRI_MODI'

# Logging
Path('results_new_fine').mkdir(exist_ok=True)
tf.logging.set_verbosity(logging.INFO)
handlers = [
    logging.FileHandler('results_new_fine/main.log'),
...
def parse_fn(line_words, line_tags):
    ...
    jasos = [[c.encode() for c in unicodedata.normalize("NFD", w)]for w in line_words.strip().split()]
    lengths_jaaso = [len(j) for j in jasos]

    max_jaso_len = max(lengths_jaaso)

    jasos = [j + [b'<pad>']* (max_jaso_len - l) for j, l in zip(jasos,lengths_jaaso)]
    return ((words, len(words)), (chars, lengths), (jasos, lengths_jaaso)), tags
...
def model_fn(features, labels, mode, params):
    ...
    variable = tf.get_variable(
        'jasos_embeddings', [num_jasos, params['dim_chars']], tf.float32)
    jaso_embeddings = tf.nn.embedding_lookup(variable, jaso_ids)
    jaso_embeddings = tf.layers.dropout(jaso_embeddings, rate=dropout,
                                        training=training)
    ...
    embeddings = tf.concat([word_embeddings, char_embeddings,jaso_embeddings], axis=-1)
...
if __name__ == '__main__':
    ...
    params = {
        ...
        'jasos' : str(Path(DATADIR, 'vocab.jasos.txt')),
        'tags': str(Path(DATADIR, 'vocab.tags.txt')),
        'fasttext': str(Path(DATADIR, 'fasttext.npz'))
    }
    with Path('results_new_fine/params.json').open('w') as f:
    ...
    estimator = tf.estimator.Estimator(model_fn, 'results_new_fine/model', cfg, params)
    ...
    def write_predictions(name):
        Path('results_new_fine/score').mkdir(parents=True, exist_ok=True)
        with Path('results_new_fine/score/{}.preds.txt'.format(name)).open('wb') as f:
            test_inpf = functools.partial(input_fn, fwords(name), ftags(name))
            golds_gen = generator_fn(fwords(name), ftags(name))
            preds_gen = estimator.predict(test_inpf)
            for golds, preds in zip(golds_gen, preds_gen):
                ((words, _), (_, _), (_, _)), tags = golds
                for word, tag, tag_pred in zip(words, tags, preds['tags']):
                    f.write(b' '.join([word, tag, tag_pred]) + b'\n')
                f.write(b'\n')

    for name in ['etri_seg_train', 'etri_seg_testa', 'etri_seg_testb']:
