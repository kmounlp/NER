...
DATADIR = '../../data/example/ETRI_TAG'

# Logging
Path('results_tag_500_3').mkdir(exist_ok=True)
tf.logging.set_verbosity(logging.INFO)
handlers = [
    logging.FileHandler('results_tag_500_3/main.log'),
...
def model_fn(features, labels, mode, params):
    ...
    embeddings = tf.concat([word_embeddings, jaso_embeddings], axis=-1)
...
if __name__ == '__main__':
    ...
    with Path('results_tag_500_3/params.json').open('w') as f:
    ...
    estimator = tf.estimator.Estimator(model_fn, 'results_tag_500_3/model', cfg, params)
    ...
    def write_predictions(name):
        Path('results_tag_500_3/score').mkdir(parents=True, exist_ok=True)
        with Path('results_tag_500_3/score/{}.preds.txt'.format(name)).open('wb') as f:
