...
DATADIR = '../../data/example/ETRI_FINAL_TAG'

# Logging
Path('result_etri_final_postag_bit').mkdir(exist_ok=True)
tf.logging.set_verbosity(logging.INFO)
handlers = [
    logging.FileHandler('result_etri_final_postag_bit/main.log'),
...
if __name__ == '__main__':
    ...
    with Path('result_etri_final_postag_bit/params.json').open('w') as f:
    ...
    estimator = tf.estimator.Estimator(model_fn, 'result_etri_final_postag_bit/model', cfg, params)
    ...
    def write_predictions(name):
        answer = []
        predict = []
        Path('result_etri_final_postag_bit/score').mkdir(parents=True, exist_ok=True)
        with Path('result_etri_final_postag_bit/score/{}.preds.txt'.format(name)).open('wb') as f:
    ...
    # with open('result_etri_final_postag_bit/socre/testb.preds.txt','r',encoding="utf8") as fb:
    #     print("computing_F1")
    #     pred = []
    #     answer = []
