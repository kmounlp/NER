...
# Logging
Path('results_kor_iter').mkdir(exist_ok=True)
tf.logging.set_verbosity(logging.INFO)
handlers = [
    logging.FileHandler('results_kor_iter/main.log'),
...
def parse_fn(line_words, line_tags):
    ...
    return ((words, len(words)), chars), tags
...
if __name__ == '__main__':
    ...
    with Path('results_kor_iter/params.json').open('w') as f:
    ...
    cfg = tf.estimator.RunConfig(save_checkpoints_secs=750, session_config=gpuconfig)
    estimator = tf.estimator.Estimator(model_fn, 'results_kor_iter/model', cfg, params)
    ...
    def write_predictions(name):
        answer = []
        predict = []
        Path('results_kor_iter/score').mkdir(parents=True, exist_ok=True)
        with Path('results_kor_iter/score/{}.preds.txt'.format(name)).open('wb') as f:
