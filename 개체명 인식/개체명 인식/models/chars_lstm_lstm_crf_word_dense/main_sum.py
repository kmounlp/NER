...
# Logging
Path('results_kor_sum').mkdir(exist_ok=True)
tf.logging.set_verbosity(logging.INFO)
handlers = [
    logging.FileHandler('results_kor_sum/main.log'),
...
def model_fn(features, labels, mode, params):
    ...
    embeddings = tf.concat([word_embeddings, char_embeddings, weight_op], axis=-1)
...
if __name__ == '__main__':
    ...
    with Path('results_kor_sum/params.json').open('w') as f:
    ...
    estimator = tf.estimator.Estimator(model_fn, 'results_kor_sum/model', cfg, params)
    ...
    def write_predictions(name):
        Path('results_kor_sum/score').mkdir(parents=True, exist_ok=True)
        with Path('results_kor_sum/score/{}.preds.txt'.format(name)).open('wb') as f:
