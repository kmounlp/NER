...
# Logging
Path('results_kor_500').mkdir(exist_ok=True)
tf.logging.set_verbosity(logging.INFO)
handlers = [
    logging.FileHandler('results_kor_500/main.log'),
...
if __name__ == '__main__':
    ...
    with Path('results_kor_500/params.json').open('w') as f:
    ...
    estimator = tf.estimator.Estimator(model_fn, 'results_kor_500/model', cfg, params)
    ...
    def write_predictions(name):
        Path('results_kor_500/score').mkdir(parents=True, exist_ok=True)
        with Path('results_kor_500/score/{}.preds.txt'.format(name)).open('wb') as f:
